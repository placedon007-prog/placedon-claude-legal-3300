...
| 1 Engine | ✅ DONE. All of `src/lib/engine/{types,errors,http,server-guard,provider,mock,index}.ts` complete. `getEngine()` reads `PLACEDON_API_ORIGIN` at call time (unset → Mock). Mock carries real Companies Act 2013 fixtures incl. a genuine s.96(1) abstention (G.S.R. 880(E)). tsc + eslint clean; all 6 fixtures validate against zod at runtime; `tests/contracts.mjs` 36 assertions pass. `.env.example` documented. STILL TODO (Layer 5/7): retire the fictional `src/lib/api.ts` once callers move to `@/lib/engine`; wire product surfaces to `getEngine()`. `server-only` npm pkg NOT installed → `server-guard.ts` shim kept (swap for real `server-only` if added). |
...
| 4 Glass | ✅ DONE (system + first applications). `.glass-1/2/3`, `.glass-abstain`, `.glass-interactive` authored in `globals.css` with the tint+brightness(0.55) discipline, `@supports` enhancement, and `prefers-reduced-transparency`/`prefers-contrast` opaque fallbacks. APPLIED: sticky nav `.dnav` (reworked off `saturate()`), demo Prompt/Connectors rail `.dcard` (glass-2), and the stage `Reveal` set to `mode="surface"` to avoid the backdrop-root trap. STILL TODO (later layers): citation/status chips → glass-1; the abstain panel → `.glass-abstain` (comes with Layer 7 product surfaces / any verdict chip); consider the `.daside` chips row. Verify ≤3 backdrop surfaces/viewport holds once more glass lands. Visual check still pending (extension was down). |
...
**DO THIS FIRST on resume:** Layers 0–4 are DONE and pushed. Next is **Layer 5 (recover the dead layer)**:
(a) wire SEO — call `pageMetadata()` in `layout.tsx` instead of hard-coded metadata, emit JSON-LD;
(b) build `/privacy` `/terms` `/cookies` routes from the existing legal `.md` + `legal-page.tsx`/`legal-document.tsx`;
(c) restore `evidence-card.tsx:85-118` APG roving-tabindex tabs into the live demo; (d) ONLY after a visual
pass, delete the ~1,150 dead globals.css + ~250 dead dashboard.css lines (risky — verify live ranges first).
`:3200` must never be modified. Both dev servers on :3300/:3200. Re-pair the browser extension for the visual pass.
